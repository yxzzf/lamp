@extends('layouts.index') @section('title') 广告浏览 @endsection @section('title','广告浏览') 
@section('content')
        ...
        <table class="mws-table">
           ...
           @foreach($gg as $k=>$v)
                <tr>
                    ...
                    <td>{{$v->name}}</td>
                    <td>{{$v->content}}</td>
                    <td><img src="{{$v->images}}" width="50" height="50"></td>
                    <td>
                        <form action="/admin/guanggao/{{$v->id}}" method="post">
                            {{method_field('DELETE')}}
                            {{ csrf_field() }}
                            <a href="/admin/guanggao/{{$v->id}}/edit" class="btn btn-success">修改</a>
